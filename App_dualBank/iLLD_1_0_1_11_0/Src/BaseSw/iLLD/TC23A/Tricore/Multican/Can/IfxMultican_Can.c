...
//    Ifx_CAN_MO *hwObj = IfxMultican_MsgObj_getPointer(msgObj->node->mcan, objId);
    hwObj123 = IfxMultican_MsgObj_getPointer(msgObj->node->mcan, objId);
    if (msgObj->msgObjCount > 1)
    {
        /* send the message */
        status = IfxMultican_MsgObj_writeTxfifo(hwObj123, msg);
    }
    else
    {
        /* send the message */
        status = IfxMultican_MsgObj_sendMessage(hwObj123, msg);
    }
    ...
    if (status == IfxMultican_Status_ok)
    {
        ...
        {
            /* set next message object(MOSTAT.PNEXT) of the current object as the next txFIFO slave object */
            msgObj->fifoPointer = IfxMultican_MsgObj_getBottomObjectPointer(hwObj123);
        }
        else
        {}
    }

...
IfxMultican_Status IfxMultican_MsgObj_sendMessage(Ifx_CAN_MO *hwObj, const IfxMultican_Message *msg)
{
    ...
    {                           /* previous message was not transferred, e.g. due to busy bus, BUS-OFF or others */
        ...
        TXRQ1count++;                    // 曾军加20210127
        if(TXRQ1count>=1000)             // 曾军加20210127
        {
//        	TXRQ1count = 0;
        	status = IfxMultican_Status_ok;   // 曾军加20210127
//          CAN9251_BusError_Flag = 1; 	// 故障标志，验证可以实现，在下面加上当前时间最好，曾军20210128
            							// 故障标志在下电时写入EEPROM，写入后清除故障标志。20221128
        }

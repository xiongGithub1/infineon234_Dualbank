#!/usr/bin/env python3
"""
ZXDoc script: Generate .pkg for Bank B (0x80100000)
...
# ============================================
HEX_FILE       = r"..\App_dualBank\debug_b\App_dualBank.hex"
BASE_ADDR      = 0x80100000      # HEX 编译基址，用于 offset 平移
PAYLOAD_ADDR   = 0x00000000      # .pkg Header 中填 0（offset 模式）
SW_VERSION     = 0x00010002
SW_MASK        = 0xFFFFFFFF
PRIV_KEY       = r".\keys\private.pem"
OUT_FILE       = r"..\tc234bootloader\App_dualBank_B.pkg"
...
def __zxdoc_main__():
    ...
    print(f"[gen_pkg_B] HEX:       {hex_path}")
    print(f"[gen_pkg_B] BaseAddr:  0x{BASE_ADDR:08X}")
    print(f"[gen_pkg_B] Key:       {key_path}")
    print(f"[gen_pkg_B] Out:       {out_path}")
    ...
    print(f"[gen_pkg_B] Done! .pkg generated: {out_path}")

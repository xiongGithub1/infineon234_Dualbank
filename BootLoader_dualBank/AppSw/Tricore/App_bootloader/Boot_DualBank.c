...
void Boot_DualBank_Init(void)
{
    ...
    if (Boot_DualBank_ReadFlags(&flags) == FALSE)
    {
        ...
        flags.main.crc32           = 0u;
        flags.main.targetWriteBank = BANK_B;  /* 默认推荐刷写 Bank B */
...
BankStatus_t Boot_DualBank_VerifyBank(uint32 bank)
{
    ...
    /* Calculate actual CRC of bank content */
    {
...
void Boot_DualBank_JumpToBank(uint32 bank)
{
    ...
    /* ========== 新增：关闭 ECC Trap，防止 App Flash 空区域触发 Trap ========== */
    {
    ...
    /* ========== 修复：同时禁用 Data Cache 和 Program Cache ========== */
    IfxCpu_setDataCache(0);      /* 禁用数据缓存 */
    IfxCpu_setProgramCache(0);   /* 禁用指令缓存（关键修复） */

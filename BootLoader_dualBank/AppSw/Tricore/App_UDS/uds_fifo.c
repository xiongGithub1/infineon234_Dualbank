...
typedef struct
{
    tId xOwnerId;                  /* Owner FIFO ID 2个字节 */
    tLen xFifoLen;                 /* FIFO len 2个字节*/
    tLen xReadAddr;                /* Read FIFO addr 2个字节*/
    tLen xWriteAddr;               /* Write FIFO addr 2个字节*/
    tFifoStatus eFifoStatus;       /* FIFO status 4个字节*/
    unsigned char *pStartFifoAddr; /* Start FIFO addr 指针类型，在32位系统上占用 4 字节*/
    void *pvNextFifoList;          /* Next FIFO list 4个字节*/
} tFifoInfo;

#define STRUCT_LEN (20u) /* Every FIFO struct used space */
#define TOTAL_BYTES ((STRUCT_LEN) * (FIFO_NUM) + TOTAL_FIFO_BYTES) /* config total bytes 20*8+1024=1184 */

static unsigned char gs_ucFifo[TOTAL_BYTES] = {0};    /* Total FIFO len  1184 */
static tFifoInfo *gs_pstListHeader = (tFifoInfo *)0u; /* Manage list FIFO header */
static tLen gs_xCleanFifoLen = TOTAL_BYTES;           /* Can used FIFO len  1184 */
...
//    CleanFIFOLenTmp = (uint32)(&gs_ucFifo[1184 - 1184]) & 0x03u;
//    &gs_ucFifo[TOTAL_BYTES - gs_xCleanFifoLen]计算指向gs_ucFifo数组中有效数据部分的起始地址。
//    (uint32)：将指针转换为无符号32位整数类型，以便进行位操作。
//    & 0x03u：这个操作用于检查该地址是否对齐到4字节边界。0x03的二进制形式是00000011，通过与这个掩码进行
//	   按位与操作，可以得到地址的最低两位。如果结果不为0，则说明地址不是4字节对齐的。
